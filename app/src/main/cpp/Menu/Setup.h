...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Dark Team</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("ESP MOD MENU | Credit : @Aryan_LK"));

    //Dialog Example
    //setDialog(ctx,env,OBFUSCATE("Title"),OBFUSCATE("Message Example"));

    //Toast Example
    Toast(env,ctx,OBFUSCATE("Menu credit: Dark Team"),ToastLength::LENGTH_LONG);
